...
grep -q 'host_vm_policy_firstboot_release_summary.py' "$WORKFLOW"
...
grep -q 'tests/test_host_vm_policy_firstboot_release_summary_static.sh' "$WORKFLOW"
...
grep -q 'Build release-readiness summary' "$WORKFLOW"
grep -q 'firstboot_release_summary.json' "$WORKFLOW"
grep -q 'firstboot_release_summary.report' "$WORKFLOW"
grep -q 'summary_ready' "$WORKFLOW"
grep -q 'expected_blocked_decision=release_receipt_blocked' "$WORKFLOW"
grep -q 'blocking_issue_count=0' "$WORKFLOW"
