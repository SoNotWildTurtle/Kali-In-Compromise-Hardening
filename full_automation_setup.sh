...
ensure_dependencies() {
    local missing=()
    for cmd in curl gpg bsdtar mkisofs isohybrid dialog nmap netdiscover arp-scan nbtscan dnsrecon whatweb enum4linux nikto sslscan snmpwalk masscan traceroute code-oss git pre-commit shellcheck flake8 bandit black codespell gitleaks isort python3 pip3; do
