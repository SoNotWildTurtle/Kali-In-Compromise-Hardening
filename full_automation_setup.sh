...
if ! command -v apt-get >/dev/null 2>&1; then
    echo "apt-get is required to bootstrap dependencies on a clean system" >&2
    exit 1
fi
...
ensure_dpkg_split() {
    if ! command -v dpkg-split >/dev/null 2>&1; then
        ...
        apt_update_once
        apt-get install -y --no-install-recommends --reinstall dpkg
    fi
}
...
ensure_dependencies() {
    ...
    # Some tooling is not present on all Debian-based distributions. Attempt to
    # install it when available but do not fail the press if the packages are
    # missing from the configured repositories.
    declare -A optional=(
    ...
    for cmd in "${!required[@]}"; do
        if ! command -v "$cmd" >/dev/null 2>&1; then
            required_pkgs+=("${required[$cmd]}")
        fi
    done
    if (( ${#required_pkgs[@]} )); then
        mapfile -t required_pkgs < <(printf '%s\n' "${required_pkgs[@]}" | LC_ALL=C sort -u)
        apt_update_once
    ...
    for cmd in "${!optional[@]}"; do
        if ! command -v "$cmd" >/dev/null 2>&1; then
            optional_pkgs+=("${optional[$cmd]}")
        fi
    done
    if (( ${#optional_pkgs[@]} )); then
        mapfile -t optional_pkgs < <(printf '%s\n' "${optional_pkgs[@]}" | LC_ALL=C sort -u)
        apt_update_once
        if ! apt-get install -y --no-install-recommends "${optional_pkgs[@]}"; then
            echo "Warning: Optional tooling could not be installed: ${optional_pkgs[*]}" >&2
        fi
    fi
}
