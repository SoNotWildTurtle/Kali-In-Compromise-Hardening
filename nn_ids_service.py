...
GAP_EPSILON = 0.001
GLOBAL_EWMA_ALPHA = 0.2
DRIFT_WARNING_DELTA = 0.05
DRIFT_CRITICAL_DELTA = 0.15
MODEL_REFRESH_DAYS = 7

ADVERSARIAL_PATHS = [
    Path("/opt/nnids/datasets/adversarial.csv"),
    Path("/opt/nnids/adversarial.csv"),
]
...
def _load_adversarial_profiles() -> List[Dict[str, Any]]:
        ...
        try:
            with path.open(newline="") as handle:
                ...
                for row in reader:
                    ...
                    canonical = reason or "Adversarial sample"
                    severity_raw = (row.get("severity") or "unknown").strip()
                    severity_key = severity_raw.lower() or "unknown"
                    severity_label = severity_raw.title() if severity_raw else "Unknown"
                    tactic = (row.get("tactic") or "Unknown").strip() or "Unknown"
                    stage = (row.get("stage") or tactic or "Unknown").strip() or "Unknown"
                    recommendation = (row.get("recommendation") or "").strip()
                    keyword_field = row.get("keywords") or ""
                    tokens: Set[str] = set()
                    template_name = (row.get("template") or "").strip()
                    for token in keyword_field.split(";"):
                        token = token.strip().lower()
                        if token:
                            tokens.add(token)
                    tokens.add(canonical.lower())
                    if template_name:
                        tokens.add(template_name.strip().lower())
                    profiles.append(
                        {
...
ADVERSARIAL_PROFILES = _load_adversarial_profiles()


def match_adversarial_profile(reason: Optional[str]) -> Optional[Dict[str, Any]]:
    if not reason:
        return None
    lowered = reason.lower()
    for profile in ADVERSARIAL_PROFILES:
        tokens = profile.get("match_tokens", set())
        for token in tokens:
...
def _record_adversarial_stats(
    data: Dict[str, Any], profile: Dict[str, Any], timestamp: datetime
) -> None:
    severity_label = profile.get("severity_label") or profile.get("severity") or "Unknown"
    severity_map = data.get("adversarial_severity") or {}
    if not isinstance(severity_map, dict):
        severity_map = {}
    severity_map[severity_label] = _as_int(severity_map.get(severity_label)) + 1
    data["adversarial_severity"] = _trim_counts(severity_map)

    stage_label = profile.get("stage") or "Unknown"
    stage_map = data.get("adversarial_stages") or {}
    if not isinstance(stage_map, dict):
        stage_map = {}
    stage_map[stage_label] = _as_int(stage_map.get(stage_label)) + 1
    data["adversarial_stages"] = _trim_counts(stage_map)

    tactic_label = profile.get("tactic") or stage_label
    tactic_map = data.get("adversarial_tactics") or {}
    if not isinstance(tactic_map, dict):
        tactic_map = {}
    tactic_map[tactic_label] = _as_int(tactic_map.get(tactic_label)) + 1
    data["adversarial_tactics"] = _trim_counts(tactic_map)

    recommendation = profile.get("recommendation")
    if recommendation:
        rec_map = data.get("adversarial_recommendations") or {}
        if not isinstance(rec_map, dict):
            rec_map = {}
        rec_map[recommendation] = _as_int(rec_map.get(recommendation)) + 1
        data["adversarial_recommendations"] = _trim_counts(rec_map)

    recent = data.get("adversarial_recent")
    if not isinstance(recent, list):
        recent = []
    recent.append(
        {
            "time": timestamp.isoformat(timespec="seconds"),
            "reason": profile.get("canonical_reason"),
            "stage": stage_label,
            "severity": severity_label,
            "recommendation": recommendation or "",
        }
    )
    data["adversarial_recent"] = _trim_list(recent)


def _decorate_reason(reason: Optional[str], profile: Optional[Dict[str, Any]]) -> str:
    if profile is None:
        return reason or "unknown pattern"
    base = reason or profile.get("canonical_reason") or "unknown pattern"
    extras: List[str] = []
    severity = profile.get("severity_label")
    if severity:
        extras.append(severity)
    stage = profile.get("stage")
    if stage and stage not in extras:
        extras.append(stage)
    tactic = profile.get("tactic")
    if tactic and tactic not in extras and tactic != stage:
        extras.append(tactic)
    if extras:
...
def update_alert_stats(
    ...
    canonical_reason = reason or "unknown pattern"
    if (
        adversarial_profile
        and isinstance(adversarial_profile, dict)
        and adversarial_profile.get("canonical_reason")
    ):
        canonical_reason = adversarial_profile["canonical_reason"]

    decorated_reason = display_reason or _decorate_reason(reason, adversarial_profile)
    reason_lower = (reason or canonical_reason).lower()
    ...
    global_prev = data.get("global_ewma_probability")
    try:
    ...
    if global_prev is None:
        global_avg = current_probability
    else:
        global_avg = GLOBAL_EWMA_ALPHA * current_probability + (1 - GLOBAL_EWMA_ALPHA) * global_prev
    data["global_ewma_probability"] = round(global_avg, 3)

    trend_delta = current_probability - global_avg
    data["global_probability_trend"] = round(trend_delta, 3)

    prob_history = data.get("probability_history")
    if not isinstance(prob_history, list):
        prob_history = []
    prob_history.append(round(current_probability, 3))
    data["probability_history"] = prob_history[-STATE_LIMIT:]

    recent_tail = prob_history[-10:]
    if recent_tail:
        recent_avg = sum(recent_tail) / len(recent_tail)
        data["recent_probability_average"] = round(recent_avg, 3)
    else:
        recent_avg = global_avg

    drift_delta = THRESHOLD - global_avg
    data["model_drift_delta"] = round(drift_delta, 3)

    low_streak = _as_int(data.get("low_probability_streak", 0))
    if current_probability < THRESHOLD:
        low_streak += 1
    else:
        low_streak = 0
    data["low_probability_streak"] = low_streak

    health = "nominal"
    if drift_delta > DRIFT_CRITICAL_DELTA or low_streak >= 6:
        health = "degraded"
    elif drift_delta > DRIFT_WARNING_DELTA or low_streak >= 3:
        health = "watch"

    if recent_avg < THRESHOLD - DRIFT_CRITICAL_DELTA:
        health = "degraded"
    elif recent_avg < THRESHOLD - DRIFT_WARNING_DELTA and health == "nominal":
        health = "watch"

    data["model_health"] = health
    ...
    data["last_reason"] = decorated_reason
    data["last_canonical_reason"] = canonical_reason
    if isinstance(adversarial_profile, dict):
        data["last_adversarial_profile"] = {
            "severity": adversarial_profile.get("severity_label")
            or adversarial_profile.get("severity"),
            "stage": adversarial_profile.get("stage"),
            "tactic": adversarial_profile.get("tactic"),
            "recommendation": adversarial_profile.get("recommendation"),
        }
    else:
        data.pop("last_adversarial_profile", None)
    ...
    if model_path.exists():
        try:
            ...
            model_last_trained = mtime.isoformat(timespec="seconds")
            age_seconds = max((now_utc - mtime).total_seconds(), 0.0)
            model_age_days = age_seconds / 86400.0
        except (OSError, ValueError):
            model_age_days = None
            model_last_trained = None
    refresh_needed = True
    if model_age_days is not None:
        model_info["age_days"] = round(model_age_days, 2)
        refresh_needed = model_age_days >= MODEL_REFRESH_DAYS
    else:
        model_info["age_days"] = None
    model_info["last_trained"] = model_last_trained
    model_info["refresh_recommended"] = bool(refresh_needed)

    stale_alerts = _as_int(model_info.get("stale_alerts", 0))
    if refresh_needed:
        stale_alerts += 1
    else:
        stale_alerts = 0
    model_info["stale_alerts"] = stale_alerts
    model_info["global_average_probability"] = data.get("global_ewma_probability")
    model_info["recent_average_probability"] = data.get("recent_probability_average")
    model_info["health"] = data.get("model_health")
    ...
    history.append(
        {
            ...
            "reason": decorated_reason,
            "canonical_reason": canonical_reason,
            "tactic": primary_tactic,
            "technique": primary_technique,
            "zero_day": bool(zero_day_hit),
            "severity": (
                adversarial_profile.get("severity_label")
                if isinstance(adversarial_profile, dict)
                else None
            ),
            "stage": (
                adversarial_profile.get("stage")
                if isinstance(adversarial_profile, dict)
                else None
            ),
            "recommendation": (
                adversarial_profile.get("recommendation")
                if isinstance(adversarial_profile, dict)
                else None
            ),
        }
...
def analyze(pkt):
    ...
    if feats:
        ...
        if pred == 1:
            ...
            adversarial_profile = match_adversarial_profile(reason)
            display_reason = _decorate_reason(reason, adversarial_profile)
            port = None
            proto_label = "other"
            if TCP in pkt:
                port = pkt[TCP].dport
            ...
            if tactic_label:
                message = (
                    f"Threat ({prob:.2f}) [{tactic_label}]: {pkt.summary()} Reason: {display_reason}"
                )
            else:
                message = f"Threat ({prob:.2f}): {pkt.summary()} Reason: {display_reason}"
            snapshot = (
                update_alert_stats(
                    prob,
                    pkt,
            ...
            if isinstance(adversarial_profile, dict):
                severity_label = adversarial_profile.get("severity_label")
                if severity_label:
                    extra_parts.append(f"Severity {severity_label}")
                recommendation = adversarial_profile.get("recommendation")
                if recommendation:
                    extra_parts.append(f"Rec: {recommendation}")
            ...
            confidence_label = "High confidence" if prob >= HIGH_CONFIDENCE_FLOOR else "Low confidence"
            meta = {
                "probability": round(float(prob), 4),
                "confidence": "high" if prob >= HIGH_CONFIDENCE_FLOOR else "low",
                "tactic": tactic_label,
                "reason": reason,
                "explanation": display_reason,
                "stage": adversarial_profile.get("stage") if isinstance(adversarial_profile, dict) else None,
                "severity": adversarial_profile.get("severity_label") if isinstance(adversarial_profile, dict) else None,
                "recommendation": adversarial_profile.get("recommendation") if isinstance(adversarial_profile, dict) else None,
                "template": adversarial_profile.get("template") if isinstance(adversarial_profile, dict) else None,
                "src": pkt[IP].src if IP in pkt else None,
                "dst": pkt[IP].dst if IP in pkt else None,
                "dport": port,
            }
            meta = {k: v for k, v in meta.items() if v not in (None, "", [])}
            log_line = f"{timestamp} | {confidence_label} | {message}"
            if meta:
                log_line += f" | meta={json.dumps(meta, sort_keys=True, separators=(',', ':'))}"
