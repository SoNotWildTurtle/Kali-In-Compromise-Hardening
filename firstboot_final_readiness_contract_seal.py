#!/usr/bin/env python3
# MINC - Defensive final-readiness contract seal gate.
# Purpose: validate aggregate final-readiness manifest smoke .summary.env evidence without sourcing shell.
...
DEFAULT_INPUT = Path('/var/log/firstboot_release_gate.final_readiness_manifest_smoke.summary.env')
SOURCE_PREFIX = 'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_'
...
REQUIRED_KEYS = {
    ...
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_COMPONENT',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_DECISION',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_BLOCKER_COUNT',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_BLOCKERS',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_EXPECTED_ARTIFACTS',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SAFE_DEFAULT',
}
...
def read_env(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f'missing_manifest_smoke_summary:{path}']
    except UnicodeDecodeError as exc:
        return {}, [f'invalid_manifest_smoke_summary_encoding:{exc.reason}']
...
def build_report(values: dict[str, str], initial_issues: list[str]) -> dict[str, Any]:
    ...
    ok_value = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_OK', 'missing')
    decision = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_DECISION', 'missing')
    release_gate = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_RELEASE_GATE', 'missing')
    source_component = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_COMPONENT', 'missing')
    source_decision = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_DECISION', 'missing')
    source_release_gate = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_RELEASE_GATE', 'missing')
    source_privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SOURCE_PRIVACY_SCOPE', 'missing')
    privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_PRIVACY_SCOPE', 'missing')
    safe_default = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_SAFE_DEFAULT', 'missing')
    blockers = values.get('FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_BLOCKERS', 'missing')
    blocker_count = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_BLOCKER_COUNT', issues)
    expected_artifacts = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_MANIFEST_SMOKE_EXPECTED_ARTIFACTS', issues)
    ...
    return {
        'schema_version': 1,
        'component': 'firstboot_final_readiness_contract_seal',
        'ok': approved,
        ...
        'operator_summary': 'Final readiness contract seal evidence is approved.' if approved else f'Final readiness contract seal evidence is deferred with {len(unique_issues)} blocker(s).',
        'privacy_scope': 'aggregate_firstboot_final_readiness_contract_seal_only',
        'safe_default': 'read-only final readiness contract seal helper; no live system state was changed',
        'rollback_note': 'remove this optional contract seal helper or stop packaging it; final-readiness manifest smoke JSON, Markdown, and summary evidence remain authoritative',
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate passive aggregate firstboot final-readiness manifest smoke summary evidence.')
