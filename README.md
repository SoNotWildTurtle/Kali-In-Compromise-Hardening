...
- **IDS Control Menu**: A terminal menu toggles malicious packet notifications, packet sanitization, automatic IP blocking, and threat feed updates, chooses follow-up discovery mode, lists or unblocks blocked IPs, views recent IDS alerts, can trigger a fresh network discovery, and is accessible via a desktop shortcut.
...
- `ids_menu.sh` – Interactive menu to toggle IDS notifications, packet sanitization, automatic IP blocking, threat feed blocking, set network discovery response modes, manage blocked IP addresses, and launch network discovery.
...
- **Configure IDS Responses**: Launch the desktop "IDS Control Menu" icon or run `ids_menu.sh` to adjust `NN_IDS_NOTIFY`, `NN_IDS_SANITIZE`, `NN_IDS_DISCOVERY_MODE` (auto|manual|notify|none), `NN_IDS_AUTOBLOCK`, and `NN_IDS_THREAT_FEED`. The menu also lets you list or unblock IP addresses, view recent IDS alerts, and kick off a new network discovery.
