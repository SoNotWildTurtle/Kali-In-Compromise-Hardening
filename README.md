...
- **Full Automation Helper**: `full_automation_setup.sh` installs the secure dev environment, prepares IDS components, and interactively collects ISO type, output location, and Kali/guest credentials before building the custom ISO.
...
- `full_automation_setup.sh` – Runs secure_dev_env, IDS setup, and the ISO builder for a turnkey build, prompting for build mode, output path, and credentials.
...
# Prompts will ask for ISO type, output location, and credentials.
```
