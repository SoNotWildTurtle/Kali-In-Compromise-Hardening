...
4. **Analytics and dashboards** expose next-generation telemetry (MITRE mapping, dwell time, burst/beacon detection, probability distributions, and more) through a boot-launched control menu and desktop shortcut with inline toggles, playbooks, and one-click response tooling.
...
- **Core IDS service:** `nn_ids_service.py` scores packets using a configurable threshold, emits ISO-8601 timestamps with structured JSON metadata (confidence, severity, stage, tactic, template, recommendation, src/dst, dport) for every alert, rotates logs, and maintains rich analytics (probability spread, TTL/length/flag distributions, streak counters, port/protocol trends, MITRE kill-chain stage totals, tactic transitions, dwell time, burst/beacon detection, risk-ranked source profiles, zero-day counters, APT watchlists, multi-tactic diversity, IPv4/IPv6 splits, subnet hotspots, hourly/minute timelines, rolling history, and more) while layering behavioural roles (scanner, brute-forcer, beaconing, lateral-mover, exfiltrator, high-risk, protocol-hopper, zero-day), fan-out and port-diversity leaders, and campaign-scoring intelligence. Adaptive EWMA baselines now flag sudden traffic surges, probability spikes, and intensity scores so high-velocity campaigns surface immediately in dashboards and alert messages, while global probability tracking computes model health/drift metrics and recommends retraining when the model ages out.
- **Continuous learning:** `nn_ids_capture.py` gathers live traffic, `nn_ids_retrain.py` merges datasets with captured samples, and `nn_ids_adversarial.py` generates annotated adversarial packets for resilient training. `setup_nn_ids.sh` verifies Python tooling, records step durations to `/var/log/nnids/setup_nn_ids.log`, creates `/opt/nnids/.setup_complete`, and logs success/failure for baseline IDS, syscall, OS, and GA Tech process model training so first-boot verification can confirm readiness.
- **Adversarial intelligence:** Synthetic samples now include severity, kill-chain stage, tactic, and recommended responses. `nn_ids_service.py` correlates live alerts against that catalog to surface severity mixes, stage concentrations, and remediation guidance directly inside the analytics JSON and operator dashboard.
- **Incident response synthesis:** `nn_ids_incident_response.py` converts alert analytics into Markdown playbooks with prioritized recommendations, campaign context, and model health notes for operators.
- **Engagement simulation & after-action:** `engagement_simulation.py` now loads custom catalogues (e.g., `/etc/nn_ids/engagement_scenarios.json`), scores scenario risk based on severity weighting, records kill-chain/tactic progression, and produces keyword-rich summaries. `engagement_post_analysis.py` ingests those summaries plus live alerts to calculate detection coverage, phase-by-phase visibility, and Markdown after-action reports.
...
- **First boot routines:** `firstboot.sh` (and the simplified `firstboot_single.sh`) apply Linux hardening, randomize MAC addresses, configure network I/O logging (ports 5775 inbound / 7557 outbound), enforce SSH access controls, schedule ClamAV/Lynis/rkhunter scans, initialize AppArmor/AIDE, seed psad, capture baseline audits, ensure `/etc/nn_ids.conf` carries required defaults, enable/verify IDS timers and services, wait for the IDS setup flag before checking artifacts, perform network discovery, launch a baseline engagement simulation plus after-action analysis, start the operator dashboard, and write `/var/log/nnids/firstboot_status.log` with tagged summaries that are also echoed to the console alongside optional post-install environment snapshots.
...
- **Dashboard everywhere:** `ids_menu.sh` is a dialog-based control center launched by cron at login and via `ids_menu.desktop`. Operators can:
  ...
  - Inspect alert analytics (including next-gen insights, behavioural role tagging, fan-out/campaign watchlists, EWMA surge ratios, probability spike leaders, intensity scoring, adversarial severity/stage mix, remediation recommendations, and new model health/drift guidance), recent history, top offenders, and probability distributions.
  - Launch/monitor GA Tech process & syscall scans, view their training logs, refresh baselines, and summarize detections.
  - Kick off engagement simulations, generate post-engagement analysis, and view the most recent report directly from the analytics submenu.
  - Generate Markdown incident response reports summarizing campaign hotspots, surge sources, and model health for rapid escalation.
...
- **Graphical overview:** `ids_dashboard_gui.py` offers a curses-based multi-view dashboard (Summary, Analytics, Timeline, Operations) that highlights service health, configuration toggles, watchlists, kill-chain activity, distributions, and recent alerts. A built-in action palette (press `?`) lets operators flip `nn_ids.conf` settings, run discovery/retraining/sanitization, refresh GA Tech models, open logs, or launch the incident-response generator without leaving the interface.
...
- **Logging clarity:** Structured logs land in `/var/log/nn_ids_alerts.log`, `/var/log/nn_ids_engagement.log`, `/var/lib/nn_ids/alert_stats.json`, `/opt/nnids/*.csv`, `/opt/nnids/analysis/*.md`, `/var/log/ga_tech_*.log`, `/var/log/inbound*.log`, `/var/log/outbound*.log`, and baseline directories. Logrotate entries prevent disk exhaustion.

### Secure Development Workflow
- **Secure dev bootstrap:** `secure_dev_env.sh` installs git, pre-commit, ShellCheck, Flake8, Bandit, Black, Codespell, Gitleaks, isort, and configures git with a generated GPG key for signed commits.
...
- **First boot automation** performs (among many others):
  ...
  - Installation of desktop shortcuts for both the IDS Control Menu and the full-screen graphical dashboard, plus a cron job to auto-launch the menu at each login.
...
- **Operator dashboard:** Run `ids_menu.sh` (or click the desktop icon) to manage IDS state, launch scans, view analytics (including MITRE kill-chain, tactic transitions, dwell time, risk scores, burst/beacon detection, etc.), manage blocked IPs, interact with GA Tech models, run network discovery, and access logs/reports. The graphical dashboard (`ids_dashboard_gui.py`) complements it with an action palette for toggles, dataset hygiene, retraining, incident-response generation, GA Tech scans, and quick log access directly from the desktop.

- **Reports & logs:**
  - IDS alerts: `/var/log/nn_ids_alerts.log`
  - IDS analytics cache: `/var/lib/nn_ids/alert_stats.json`
  - Incident response reports: `/var/log/nn_ids/incident_response_report.md`
...
- `ids_dashboard.desktop` – Desktop launcher for the full-screen curses analytics dashboard.
- `ids_dashboard_gui.py` – Multi-view (Summary/Analytics/Timeline/Operations) curses dashboard summarizing IDS health, watchlists, alerts, and playbooks with a built-in action palette for toggles and response tooling.
...
- `engagement_simulation.py` – Generate multi-phase adversarial engagement datasets with per-phase metadata, severity-weighted risk scoring, kill-chain/tactic sequencing, keyword statistics, and optional external scenario catalogues.
- `engagement_post_analysis.py` – Parse IDS alert logs (and latest simulations) into Markdown after-action reports with detection coverage, phase visibility, and recommendation summaries.
...
- `nn_ids_adversarial.py` – Generate annotated adversarial packets (with severity/stage/tactic metadata and remediation guidance) for hardening the IDS model.
...
- `nn_ids_incident_response.py` – Generate Markdown incident response reports with prioritized actions, campaign context, and model health summaries.
