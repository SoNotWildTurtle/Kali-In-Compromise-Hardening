...
- **Full Automation Helper**: `full_automation_setup.sh` installs the secure dev environment, prepares IDS components, and builds the custom ISO. It prompts only for missing build mode, output path, working directory, or credentials; if all are supplied via flags, environment variables, or a `press.conf` file, it runs unattended. Defaults select installer mode, drop the ISO in the current directory, and use `/tmp/kali-auto-build` for workspace. `-a` forces unattended mode and errors if required values are missing, `-c` chooses a different config file, `PRESS_CONF` can point to a config file as well, and `-k` keeps the working directory. After an interactive run it can save the answers to `press.conf` for future automated presses. Every run writes a timestamped log file to the current directory and reports the ISO's SHA256 checksum for auditing.
...
- **IDS Control Menu**: A dialog-based dashboard toggles malicious packet notifications, packet sanitization, automatic IP blocking, and threat feed updates; chooses follow-up discovery mode; lists or unblocks blocked IPs; displays recent IDS alerts; can trigger a fresh network discovery; launch dataset sanitization or model retraining; manually refresh the threat feed; and launches on boot via cron or the desktop shortcut.
- **Secure Coding Environment**: Optional script installs Visual Studio Code (code-oss) and configures git with secure defaults. Static analysis and secret-scanning tools (Black, Flake8, Bandit, ShellCheck, isort, Codespell, and Gitleaks) run via pre-commit hooks, and a GPG key is generated so commits are signed automatically.
...
- `ids_menu.sh` – Interactive menu to toggle IDS notifications, packet sanitization, automatic IP blocking, threat feed blocking, set network discovery response modes, manage blocked IP addresses, launch network discovery, trigger dataset sanitization, or retrain the model. A cron job launches the menu at boot.
...
- `packet_sanitizer.py` – Utility for cleansing datasets before model training.
 - `/etc/nn_ids.conf` – Configuration file controlling IDS options like packet sanitization, notification, discovery response, automatic IP blocking, and threat feed blocking.
...
- `secure_dev_env.sh` – Installs Visual Studio Code, secure git defaults, and pre-commit tooling (Black, Flake8, Bandit, ShellCheck, isort, Codespell, Gitleaks), and auto-generates a GPG key for signed commits.
- `.pre-commit-config.yaml` – Global configuration enabling Black, Flake8, Bandit, ShellCheck, isort, Codespell, and Gitleaks.
- `build_custom_iso.sh` – Downloads the latest Kali installer or live ISO, verifies its SHA256 checksum and GPG signature, installs any missing build dependencies, and packages the hardening scripts into a custom image. It prompts for build mode, ISO path, working directory, and credentials if they are not supplied, accepts the same values via environment variables or a `press.conf` configuration file for unattended use, automatically creates the output directory to avoid `could not create file` errors, and after an interactive run can write the answers to `press.conf` for future automation. Each run logs to a timestamped file, reuses a cached ISO if the checksum matches, and flags `-c` and `-k` allow choosing an alternate config file and keeping the temporary working directory.
  - `full_automation_setup.sh` – Runs `secure_dev_env.sh`, IDS setup, and the ISO builder for a turnkey build. Prompts allow changing build mode, output path, working directory, and credentials, with sensible defaults if left blank. If every option is provided via flags, environment variables, or `press.conf`, the script runs without interaction. Use `-a` to force unattended execution and fail if required values are missing. When a config file is supplied, it is also passed to `build_custom_iso.sh` so both helpers share the same settings. After building, the script prints the ISO's SHA256 checksum for verification.
...
- **Configure IDS Responses**: Launch the desktop "IDS Control Menu" icon or run `ids_menu.sh` to adjust `NN_IDS_NOTIFY`, `NN_IDS_SANITIZE`, `NN_IDS_DISCOVERY_MODE` (auto|manual|notify|none), `NN_IDS_AUTOBLOCK`, and `NN_IDS_THREAT_FEED`. The menu also lets you list or unblock IP addresses, view recent IDS alerts, kick off a new network discovery, sanitize datasets, retrain the model, or update the threat feed. A cron job runs the dashboard at boot.
