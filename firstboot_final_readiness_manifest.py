#!/usr/bin/env python3
# MINC - Defensive firstboot final readiness manifest helper.
# Purpose: compose aggregate final-readiness smoke sidecar evidence into a review manifest.
...
DEFAULT_INPUT = Path('/var/log/firstboot_release_gate.final_readiness_smoke.summary.env')
SOURCE_PREFIX = 'FIRSTBOOT_FINAL_READINESS_SMOKE_'
SUMMARY_PREFIX = 'FIRSTBOOT_FINAL_READINESS_MANIFEST'
EXPECTED_SOURCE_COMPONENT = 'firstboot_final_readiness_smoke'
EXPECTED_PRIVACY_SCOPE = 'aggregate_firstboot_final_readiness_smoke_only'
REQUIRED_KEYS = {
    'FIRSTBOOT_FINAL_READINESS_SMOKE_OK',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_DECISION',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_COMPONENT',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_DECISION',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_BLOCKER_COUNT',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_BLOCKERS',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_TOTAL_ARTIFACTS',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_SMOKE_SAFE_DEFAULT',
}
EXPECTED_ARTIFACTS = (
...
def read_env(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f'missing_final_readiness_smoke_summary:{path}']
    except UnicodeDecodeError as exc:
        return {}, [f'invalid_final_readiness_smoke_summary_encoding:{exc.reason}']
...
def build_report(values: dict[str, str], initial_issues: list[str]) -> dict[str, Any]:
    ...
    ok_value = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_OK', 'missing')
    decision = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_DECISION', 'missing')
    release_gate = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_RELEASE_GATE', 'missing')
    source_component = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_COMPONENT', 'missing')
    source_decision = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_DECISION', 'missing')
    source_release_gate = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_RELEASE_GATE', 'missing')
    source_privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_SOURCE_PRIVACY_SCOPE', 'missing')
    privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_PRIVACY_SCOPE', 'missing')
    safe_default = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_SAFE_DEFAULT', 'missing')
    blockers = values.get('FIRSTBOOT_FINAL_READINESS_SMOKE_BLOCKERS', 'missing')
    blocker_count = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_SMOKE_BLOCKER_COUNT', issues)
    total_artifacts = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_SMOKE_TOTAL_ARTIFACTS', issues)
    ...
    if source_privacy_scope != EXPECTED_PRIVACY_SCOPE:
    ...
    return {
        'schema_version': 1,
        'component': 'firstboot_final_readiness_manifest',
        'ok': approved,
        ...
        'operator_summary': 'Final readiness manifest is approved from aggregate smoke evidence.' if approved else f'Final readiness manifest is deferred with {len(unique_issues)} blocker(s).',
        'blockers': unique_issues,
        'privacy_scope': 'aggregate_firstboot_final_readiness_manifest_only',
        'safe_default': 'read-only final readiness manifest helper; no live system state was changed',
        'rollback_note': 'remove this optional manifest helper or stop packaging it; final-readiness and smoke evidence remain authoritative',
    }
...
def render_summary_env(report: dict[str, Any]) -> str:
    ...
    values = {
        ...
        f'{SUMMARY_PREFIX}_EXPECTED_ARTIFACTS': len(report['expected_artifacts']),
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        '# Firstboot final readiness manifest',
        '',
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Compose passive aggregate firstboot final-readiness manifest evidence.')
