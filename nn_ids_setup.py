...
def train_model() -> None:
    ...
    adv_path = DATA_DIR / "adversarial.csv"
    adv_df.to_csv(adv_path, index=False)
    if {"severity", "stage"}.issubset(adv_df.columns):
        print("Adversarial dataset metadata summary:")
        for severity, count in adv_df["severity"].value_counts().items():
            print(f"  - {severity}: {count} samples")
        for stage, count in adv_df["stage"].value_counts().items():
            print(f"  - Stage {stage}: {count} samples")
    training_cols = ["len", "ttl", "dport", "flags", "label"]
    df = pd.concat([df, adv_df[training_cols]], ignore_index=True)
