#!/usr/bin/env python3
# MINC - Defensive firstboot final readiness release-gate helper.
# Purpose: derive aggregate-only final readiness from handoff env-policy smoke evidence.
...
DEFAULT_INPUT = Path('/var/log/firstboot_release_gate.handoff_env_policy_smoke.summary.env')
SOURCE_PREFIX = 'FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_'
SUMMARY_PREFIX = 'FIRSTBOOT_FINAL_READINESS'
EXPECTED_SOURCE_COMPONENT = 'firstboot_release_gate_handoff_env_policy_smoke'
EXPECTED_PRIVACY_SCOPE = 'aggregate_release_gate_handoff_env_policy_smoke_only'
REQUIRED_KEYS = {
...
def read_env(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f'missing_env_policy_smoke_summary:{path}']
    except UnicodeDecodeError as exc:
        return {}, [f'invalid_env_policy_smoke_summary_encoding:{exc.reason}']
...
def build_report(values: dict[str, str], initial_issues: list[str]) -> dict[str, Any]:
    ...
    ok_value = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_OK', 'missing')
    decision = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_DECISION', 'missing')
    release_gate = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_RELEASE_GATE', 'missing')
    source_component = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_SOURCE_COMPONENT', 'missing')
    source_decision = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_SOURCE_DECISION', 'missing')
    source_release_gate = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_SOURCE_RELEASE_GATE', 'missing')
    source_privacy_scope = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_SOURCE_PRIVACY_SCOPE', 'missing')
    privacy_scope = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_PRIVACY_SCOPE', 'missing')
    safe_default = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_SAFE_DEFAULT', 'missing')
    blockers = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_BLOCKERS', 'missing')
    blocker_count = as_nonnegative_int(values, 'FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_BLOCKER_COUNT', issues)
    total_artifacts = as_nonnegative_int(values, 'FIRSTBOOT_HANDOFF_ENV_POLICY_SMOKE_TOTAL_ARTIFACTS', issues)
    ...
    return {
        'schema_version': 1,
        'component': 'firstboot_final_readiness',
        'ok': approved,
        ...
        'operator_summary': 'Final readiness is approved from aggregate handoff smoke evidence.' if approved else f'Final readiness is deferred with {len(unique_issues)} blocker(s).',
        'privacy_scope': 'aggregate_firstboot_final_readiness_only',
        'safe_default': 'read-only final readiness helper; no live system state was changed',
        'rollback_note': 'remove this optional helper or stop packaging it; upstream env-policy smoke evidence remains authoritative',
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Derive passive aggregate firstboot final readiness evidence.')
