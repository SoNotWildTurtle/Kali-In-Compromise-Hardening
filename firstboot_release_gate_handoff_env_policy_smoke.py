...
REQUIRED_KEYS = {
    'FIRSTBOOT_HANDOFF_ENV_POLICY_OK',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_DECISION',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_RELEASE_GATE',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_COMPONENT',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_DECISION',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_RELEASE_GATE',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_PRIVACY_SCOPE',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_BLOCKER_COUNT',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_BLOCKERS',
    'FIRSTBOOT_HANDOFF_ENV_POLICY_TOTAL_ARTIFACTS',
...
def read_env(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f'missing_env_policy_summary:{path}']
    except UnicodeDecodeError as exc:
        return {}, [f'invalid_env_policy_summary_encoding:{exc.reason}']
...
def build_report(values: dict[str, str], initial_issues: list[str]) -> dict[str, Any]:
    ...
    ok_value = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_OK', 'missing')
    decision = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_DECISION', 'missing')
    release_gate = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_RELEASE_GATE', 'missing')
    source_component = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_COMPONENT', 'missing')
    source_decision = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_DECISION', 'missing')
    source_release_gate = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_RELEASE_GATE', 'missing')
    source_privacy_scope = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SOURCE_PRIVACY_SCOPE', 'missing')
    privacy_scope = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_PRIVACY_SCOPE', 'missing')
    safe_default = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_SAFE_DEFAULT', 'missing')
    blockers = values.get('FIRSTBOOT_HANDOFF_ENV_POLICY_BLOCKERS', 'missing')
    blocker_count = as_nonnegative_int(values, 'FIRSTBOOT_HANDOFF_ENV_POLICY_BLOCKER_COUNT', issues)
    total_artifacts = as_nonnegative_int(values, 'FIRSTBOOT_HANDOFF_ENV_POLICY_TOTAL_ARTIFACTS', issues)
    ...
    if source_component != 'firstboot_release_gate_handoff_env_policy':
    ...
    if source_privacy_scope != EXPECTED_UPSTREAM_PRIVACY_SCOPE:
    ...
    return {
        'schema_version': 1,
        'component': 'firstboot_release_gate_handoff_env_policy_smoke',
        'ok': approved,
        ...
        'operator_summary': 'Env-policy summary evidence is smoke-approved for aggregate handoff review.' if approved else f'Env-policy summary evidence is deferred with {len(unique_issues)} blocker(s).',
        'privacy_scope': 'aggregate_release_gate_handoff_env_policy_smoke_only',
        'safe_default': 'read-only summary smoke validator; no live system state was changed',
        'rollback_note': 'remove this optional smoke validator or stop packaging it; env-policy JSON and Markdown evidence remain authoritative',
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Smoke-validate aggregate firstboot handoff env-policy summary evidence.')
