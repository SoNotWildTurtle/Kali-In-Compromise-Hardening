...
- Added pytest coverage for approved evidence, mismatched release-gate evidence, blocker-count mismatch behavior, Markdown privacy/rollback output, and packaging/service/docs contracts.
...
- Remove the optional `firstboot_release_gate.service` `ExecStartPost=` lines that write env-policy JSON and Markdown artifacts.
- Delete optional generated env-policy JSON or Markdown files.
