...
`firstboot_release_gate_bundle_manifest.py` creates a passive, privacy-safe manifest for the firstboot release-gate evidence set. It records required artifact presence, sizes, and SHA-256 hashes for the JSON gate, Markdown gate, shell summary, and status-reader JSON output.

The helper is intended for ISO promotion, recovery bundle review, release handoff, and operations dashboards that need one machine-readable evidence index without embedding raw telemetry or changing live system state.
...
- Machine-readable `blockers` and privacy-safe `operator_next_steps`.
- `privacy_scope`, `privacy_exclusions`, `safe_default`, and `rollback_note` fields.

## Threat-model rationale

Evidence bundles are useful for release handoff, but bundling tools can accidentally collect raw logs, captures, credentials, hostnames, usernames, model binaries, or datasets. This helper avoids that class of failure by recording only artifact references, sizes, and hashes. It does not read the authoritative JSON or Markdown contents beyond hashing bytes, and it validates only the aggregate status-reader JSON.
