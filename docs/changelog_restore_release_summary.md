...
- The summary can be reverted independently because it only adds reviewer evidence, documentation, README navigation, and test coverage.
...
Revert this changelog, `host_vm_policy_restore_release_summary.py`, `tests/test_host_vm_policy_restore_release_summary_static.sh`, `docs/host_vm_policy_restore_release_summary.md`, and the README navigation entry. No live system state requires rollback.

## Follow-up

- Wire the restore summary into hosted restore executor release evidence after standalone static coverage is green.
