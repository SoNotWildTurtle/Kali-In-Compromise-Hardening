...
- Added optional `--summary` output for a shell-friendly `.env` status artifact that release scripts, dashboards, systemd follow-up units, and CI gates can consume without parsing full JSON or Markdown evidence.
...
- Wired `firstboot_release_gate.service` to refresh JSON, Markdown, and `.summary.env` handoff freshness artifacts after handoff verification artifacts are generated.
- Added tests for current evidence approval, stale verified artifact fail-closed behavior, Markdown handoff evidence, summary artifact generation, documentation coverage, privacy exclusions, rollback guidance, packaging coverage, and service refresh coverage.
- Documented usage, firstboot service integration, output contract, summary contract, threat-model rationale, compatibility notes, rollback guidance, and follow-up work in `docs/firstboot_release_gate_handoff_freshness.md`.

## Security

- The freshness helper is additive and passive: it reads only existing privacy-safe aggregate handoff verification evidence and filesystem metadata, then emits derived JSON, Markdown, or shell-friendly summary evidence.
...
- The summary output is aggregate-only and intentionally excludes raw logs, packet captures, credentials, hostnames, usernames, model binaries, datasets, secrets, and environment identifiers.
