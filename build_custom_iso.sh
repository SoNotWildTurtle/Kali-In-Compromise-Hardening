...
if [[ "$MODE" == "installer" ]]; then
    ISO_NAME=$(curl -fsSL --retry 3 --retry-delay 5 "$BASE_URL/" | grep -oP 'kali-linux-.*-installer-amd64\.iso' | head -n 1)
    NEED_WIN=false
    FIRSTBOOT="firstboot_single.sh"
    PRESEED_SRC="kali-preseed-single.cfg"
elif [[ "$MODE" == "live" ]]; then
    ISO_NAME=$(curl -fsSL --retry 3 --retry-delay 5 "$BASE_URL/" | grep -oP 'kali-linux-.*-live-amd64\.iso' | head -n 1)
    NEED_WIN=true
...
missing_pkgs=()
for cmd in curl bsdtar mkisofs isohybrid gpg dialog nmap netdiscover arp-scan nbtscan dnsrecon whatweb enum4linux nikto sslscan snmpwalk masscan traceroute code-oss git pre-commit shellcheck flake8 bandit black codespell gitleaks isort python3 pip3; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        case "$cmd" in
            ...
            code-oss)    missing_pkgs+=(code-oss) ;;
...
for cmd in curl bsdtar mkisofs isohybrid gpg dialog nmap netdiscover arp-scan nbtscan dnsrecon whatweb enum4linux nikto sslscan snmpwalk masscan traceroute code-oss git pre-commit shellcheck flake8 bandit black codespell gitleaks isort; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
