...
# Generate an environment report for auditing if the helper is available.
...
# Ensure curl and gnupg exist before any network operations
ensure_basic_tools() {
    local missing=()
...
# Verify that required files are embedded within the final ISO
verify_iso_contents() {
    local iso=$1
    shift
    bsdtar -tf "$iso" >/dev/null || {
        echo "Unable to read $iso" >&2
        return 1
    }
    local missing=()
    for f in "$@"; do
        if ! bsdtar -tf "$iso" | grep -q "$f"; then
            missing+=("$f")
        fi
    done
    if (( ${#missing[@]} )); then
        echo "ISO missing expected files: ${missing[*]}" >&2
        return 1
    fi
}

# Download a file with resume support while displaying progress and waiting for completion
download_iso() {
    local url=$1 dest=$2 tmp="${dest}.part" size
    rm -f "$tmp"
    echo "Downloading $url"
    curl -fL --retry 5 --retry-delay 5 -C - -o "$tmp" "$url" &
    local pid=$!
    while kill -0 $pid 2>/dev/null; do
        if [[ -f "$tmp" ]]; then
            size=$(du -h "$tmp" | cut -f1)
            printf '\rDownloaded %s' "$size"
        fi
        sleep 2
    done
    if wait $pid; then
...
NN_PROC_GT="nn_process_gt.py"
NN_SYSCALL="nn_syscall_gt.py"
NN_SYSCALL_MON="nn_syscall_monitor.py"
NN_SYSCALL_SVC="nn_syscall_monitor.service"
...
IDS_GUI="ids_dashboard_gui.py"
INCIDENT_RESP="nn_ids_incident_response.py"
IDS_GUI_DESKTOP="ids_dashboard.desktop"
...
ENGAGE_SIM="engagement_simulation.py"
ENGAGE_POST="engagement_post_analysis.py"
...
# Import the Kali archive signing key only if it is not already present
if ! gpg --list-keys | grep -q "Kali Linux"; then
    curl -fsSL --retry 3 --retry-delay 5 https://archive.kali.org/archive-key.asc -o "$WORKDIR/archive-key.asc"
    gpg --import "$WORKDIR/archive-key.asc"
fi
...
if [[ -f "$ORIG_ISO" ]]; then
    ...
    if [[ "$actual_hash" == "$expected_hash" ]]; then
        ...
        download_iso "$BASE_URL/$ISO_NAME" "$ORIG_ISO"
    fi
else
    download_iso "$BASE_URL/$ISO_NAME" "$ORIG_ISO"
fi
...
if [[ ! -s "$ORIG_ISO" ]]; then
    echo "ISO download failed: $ORIG_ISO missing" >&2
    exit 1
fi
echo "ISO $ORIG_ISO downloaded and verified"
...
cp "$NN_PROC_GT" "$INSTALL_DIR/"
cp "$NN_SYSCALL" "$INSTALL_DIR/"
cp "$NN_SYSCALL_MON" "$INSTALL_DIR/"
cp "$NN_SYSCALL_SVC" "$INSTALL_DIR/"
...
cp "$ENGAGE_SIM" "$INSTALL_DIR/"
cp "$ENGAGE_POST" "$INSTALL_DIR/"
cp "$IDS_MENU" "$INSTALL_DIR/"
cp "$IDS_GUI" "$INSTALL_DIR/"
cp "$INCIDENT_RESP" "$INSTALL_DIR/"
cp "$SSH_ACL" "$INSTALL_DIR/"
cp "$SSH_WHITE" "$INSTALL_DIR/"
cp "$SSH_BLACK" "$INSTALL_DIR/"
cp "$IDS_DESKTOP" "$INSTALL_DIR/"
cp "$IDS_GUI_DESKTOP" "$INSTALL_DIR/"
...
# Verify critical files exist in the working directory before ISO creation
for f in "$WORKDIR/preseed/kali-preseed.cfg" "$INSTALL_DIR/$(basename "$FIRSTBOOT")"; do
    if [[ ! -f "$f" ]]; then
        echo "Expected file $f missing before ISO creation" >&2
        exit 1
    fi
done
...
# Ensure expected files are present within the new ISO
verify_iso_contents "$OUT_ISO" \
    "preseed/kali-preseed.cfg" \
    "install/$(basename "$FIRSTBOOT")" \
    "install/$(basename "$DEV_SETUP")" || {
