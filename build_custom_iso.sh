...
curl -s "$BASE_URL/SHA256SUMS" -o "$WORKDIR/SHA256SUMS"
curl -s "$BASE_URL/SHA256SUMS.gpg" -o "$WORKDIR/SHA256SUMS.gpg"
curl -s https://archive.kali.org/archive-key.asc | gpg --import
