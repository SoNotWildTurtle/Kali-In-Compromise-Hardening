...
- Added `examples/nn_ids_triage_bundle_manifest.example.json` so reviewers have a stable aggregate-only manifest shape to compare against without live telemetry, secrets, identifiers, or response authority.
...
- The manifest example is static documentation evidence only; it keeps human review required, live action unauthorized, and aggregate-only privacy boundaries.
...
- Triage bundle manifest behavior and example shape are covered by `bash tests/test_nn_ids_triage_bundle_manifest_static.sh`.
