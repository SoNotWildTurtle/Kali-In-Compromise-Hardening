...
if [ -n "$NET_RANGE" ]; then
    ...
    if command -v masscan >/dev/null 2>&1; then
        masscan -p1-65535 "$NET_RANGE" --rate=1000 > "$OUTPUT_DIR/masscan.txt" || true
    fi
    ...
    for ip in $HOSTS; do
        ...
        command -v nmap >/dev/null 2>&1 && nmap --script vuln "$ip" > "$OUTPUT_DIR/nmap_vuln_${ip}.txt"
        command -v nikto >/dev/null 2>&1 && nikto -host "$ip" > "$OUTPUT_DIR/nikto_${ip}.txt"
        command -v sslscan >/dev/null 2>&1 && sslscan "$ip" > "$OUTPUT_DIR/sslscan_${ip}.txt"
        command -v snmpwalk >/dev/null 2>&1 && snmpwalk -v2c -c public "$ip" > "$OUTPUT_DIR/snmpwalk_${ip}.txt" 2>/dev/null
    done
fi
