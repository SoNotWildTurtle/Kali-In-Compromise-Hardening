...
DEFAULT_SUMMARY = Path('/var/log/firstboot_release_gate.handoff_freshness.summary.env')
...
def render_summary_env(freshness: Dict[str, Any]) -> str:
    ...
    values = {
        'FIRSTBOOT_HANDOFF_FRESHNESS_COMPONENT': freshness['component'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_CREATED_UTC': freshness['created_utc'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_OK': '1' if freshness['ok'] else '0',
        'FIRSTBOOT_HANDOFF_FRESHNESS_DECISION': freshness['decision'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_RELEASE_GATE': freshness['release_gate'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_INPUT': freshness['input_path'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_MAX_AGE_MINUTES': policy['max_age_minutes'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_TOTAL_ARTIFACTS': counts['total'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_REQUIRED_VERIFIED': counts['required_verified'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_FRESH_REQUIRED_VERIFIED': counts['fresh_required_verified'],
        'FIRSTBOOT_HANDOFF_FRESHNESS_VERIFICATION_EXISTS': '1' if verification['exists'] else '0',
        'FIRSTBOOT_HANDOFF_FRESHNESS_VERIFICATION_FRESH': '1' if verification['fresh'] else '0',
        'FIRSTBOOT_HANDOFF_FRESHNESS_BLOCKER_COUNT': len(blockers),
        'FIRSTBOOT_HANDOFF_FRESHNESS_BLOCKERS': ','.join(blockers) if blockers else 'none',
        'FIRSTBOOT_HANDOFF_FRESHNESS_PRIVACY_SCOPE': freshness['privacy_scope'],
    }
    lines = [
...
def write_summary(path: Path, freshness: Dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(render_summary_env(freshness), encoding='utf-8')
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument('--summary', default=None, help='Optional shell-friendly .env summary output path.')
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.summary:
        write_summary(Path(args.summary), freshness)
