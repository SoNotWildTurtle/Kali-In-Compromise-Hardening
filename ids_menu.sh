...
INCIDENT_SCRIPT="/usr/local/bin/nn_ids_incident_response.py"
INCIDENT_REPORT="/var/log/nn_ids/incident_response_report.md"
...
if not path.exists():
    ...
    try:
        ...
        def format_float_map(entries):
            formatted = []
            if isinstance(entries, dict):
                for key, value in entries.items():
                    try:
                        formatted.append((str(key), float(value)))
                    except (TypeError, ValueError):
                        continue
            formatted.sort(key=lambda item: item[1], reverse=True)
            formatted = formatted[:5]
            if not formatted:
                return ['  (none)']
            return [f"  - {label}: {value:.2f}" for label, value in formatted]

        def format_role_counts(entries):
            formatted = []
            if isinstance(entries, dict):
                for key, value in entries.items():
                    try:
            ...
            if not formatted:
                return ['  (none)']
            return [f"  - {label}: {count}" for label, count in formatted]

        def format_recent_adversarial(entries):
            ...
            for item in entries[-5:]:
                ...
                line = f"  - {ts} | {reason} | {severity}"
                if stage:
                    line += f" | stage {stage}"
                if recommendation:
                    line += f" | rec: {recommendation}"
        ...
        def format_profiles(entries):
            ...
            for src, profile in entries.items():
                ...
                fanout = safe_int(profile.get('unique_targets'), 0)
                port_mix = safe_int(profile.get('unique_ports'), 0)
                roles = profile.get('roles')
                if isinstance(roles, list) and roles:
                    role_info = f", roles: {', '.join(str(role) for role in roles if role)}"
                else:
                    role_info = ''
                line = (
                    f"  - {src}: risk {risk:.2f}, alerts {count}, avg {avg_prob:.2f}, "
                    f"tactic {tactic}, zero-day {zero_hits}, bursts {bursts}, "
                    f"beacons {beacons}, fan-out {fanout}, ports {port_mix}, last {last_seen}, "
                    f"proto {proto}{port_info}{role_info}"
                )
        ...
        try:
            global_avg = float(data.get('global_ewma_probability'))
        except (TypeError, ValueError):
            global_avg = None
        try:
            recent_avg = float(data.get('recent_probability_average'))
        except (TypeError, ValueError):
            recent_avg = None
        try:
            drift_delta = float(data.get('model_drift_delta'))
        except (TypeError, ValueError):
            drift_delta = None
        try:
            global_trend = float(data.get('global_probability_trend'))
        except (TypeError, ValueError):
            global_trend = None
        try:
            low_prob_streak = int(data.get('low_probability_streak', 0))
        except (TypeError, ValueError):
            low_prob_streak = 0
        ...
        if not isinstance(model_info, dict):
            model_info = {}
        last_trained = model_info.get('last_trained') or 'unknown'
        try:
            model_age_days = float(model_info.get('age_days'))
        except (TypeError, ValueError):
            model_age_days = None
        refresh_recommended = bool(model_info.get('refresh_recommended'))
        model_health = model_info.get('health') or data.get('model_health') or 'unknown'
        try:
            stale_alerts = int(model_info.get('stale_alerts', 0))
        except (TypeError, ValueError):
            stale_alerts = 0
        ...
        lines.extend([
            ...
            '',
            'Model performance:',
            f"  Health: {model_health}",
            f"  Low-probability streak: {low_prob_streak}",
        ]
        if global_avg is not None:
            lines.append(f"  Global EWMA probability: {global_avg:.3f}")
        if recent_avg is not None:
            lines.append(f"  Recent 10-alert average: {recent_avg:.3f}")
        if global_trend is not None:
            lines.append(f"  Latest trend delta: {global_trend:+.3f}")
        if drift_delta is not None:
            lines.append(f"  Drift delta (threshold - EWMA): {drift_delta:+.3f}")
        lines.extend([
            f"  Last trained: {last_trained}",
            f"  Model age (days): {model_age_days:.1f}" if model_age_days is not None else "  Model age (days): unknown",
            f"  Refresh recommended: {'yes' if refresh_recommended else 'no'}",
            f"  Alerts since refresh warning: {stale_alerts}",
            '',
            'Top source IPs:',
        ...
        fanout_lines = format_top(data.get('fanout_sources', {}))
        port_div_lines = format_top(data.get('port_diversity_sources', {}))
        campaign_lines = format_float_map(data.get('campaign_watchlist', {}))
        surge_lines = format_float_map(data.get('surge_sources', {}))
        spike_lines = format_float_map(data.get('probability_spike_sources', {}))
        intensity_lines = format_float_map(data.get('intensity_watchlist', {}))
        role_lines = format_role_counts(data.get('role_counts', {}))
        adv_severity_lines = format_top(data.get('adversarial_severity', {}))
        adv_stage_lines = format_top(data.get('adversarial_stages', {}))
        adv_tactic_lines = format_top(data.get('adversarial_tactics', {}))
        adv_rec_lines = format_top(data.get('adversarial_recommendations', {}))
        adv_recent_lines = format_recent_adversarial(data.get('adversarial_recent'))
        ...
        lines.extend(build_section('High fan-out sources', fanout_lines))
        lines.extend(build_section('Port-diversity leaders', port_div_lines))
        lines.extend(build_section('Campaign escalation watchlist', campaign_lines))
        lines.extend(build_section('Traffic surge sources', surge_lines))
        lines.extend(build_section('Probability spike leaders', spike_lines))
        lines.extend(build_section('Intensity watchlist', intensity_lines))
        lines.extend(build_section('Role distribution', role_lines))
        lines.extend(build_section('Adversarial severity mix', adv_severity_lines))
        lines.extend(build_section('Adversarial kill-chain stages', adv_stage_lines))
        lines.extend(build_section('Adversarial tactics', adv_tactic_lines))
        lines.extend(build_section('Adversarial recommendations', adv_rec_lines))

        lines.extend(['', 'Recent adversarial matches:', *adv_recent_lines])
...
if not path.exists():
    ...
    try:
        ...
        adv_severity = data.get('adversarial_severity') or {}
        if isinstance(adv_severity, dict) and adv_severity:
            lines.append('- Adversarial severity mix:')
            for severity, count in sorted(
                adv_severity.items(), key=lambda item: safe_int(item[1]), reverse=True
            )[:3]:
                lines.append(f"  * {severity}: {safe_int(count)} matches")
        else:
            lines.append('- Adversarial severity mix: none recorded yet')

        adv_recommendations = data.get('adversarial_recommendations') or {}
        if isinstance(adv_recommendations, dict) and adv_recommendations:
            top_rec = max(
                adv_recommendations.items(), key=lambda item: safe_int(item[1])
            )
            lines.append(
                f"- Most common adversarial response: {top_rec[0]}"
                f" ({safe_int(top_rec[1])} prompts)"
            )

        last_adv = data.get('last_adversarial_profile') or {}
        if isinstance(last_adv, dict) and any(last_adv.values()):
            stage = last_adv.get('stage') or 'Unknown stage'
            severity = last_adv.get('severity') or 'Unknown severity'
            tactic = last_adv.get('tactic') or 'Unknown tactic'
            recommendation = last_adv.get('recommendation') or 'No recommendation logged'
            lines.append(
                f"- Latest adversarial classification: {severity} / {stage} / {tactic}"
            )
        ...
        fanout_sources = data.get('fanout_sources') or {}
        fanout_flag = False
        if isinstance(fanout_sources, dict) and fanout_sources:
            lines.append('- High fan-out sources:')
            for src, count in sorted(
                fanout_sources.items(),
                key=lambda item: safe_int(item[1]),
                reverse=True,
            )[:3]:
                lines.append(f"  * {src} ({safe_int(count)} distinct targets)")
            fanout_flag = True
        else:
            lines.append('- High fan-out sources: none observed')

        port_mix = data.get('port_diversity_sources') or {}
        port_flag = False
        if isinstance(port_mix, dict) and port_mix:
            lines.append('- Port-diversity leaders:')
            for src, count in sorted(
                port_mix.items(),
                key=lambda item: safe_int(item[1]),
                reverse=True,
            )[:3]:
                lines.append(f"  * {src} ({safe_int(count)} unique ports)")
            port_flag = True
        else:
            lines.append('- Port-diversity leaders: none observed')

        campaign_watch = data.get('campaign_watchlist') or {}
        campaign_flag = False
        if isinstance(campaign_watch, dict) and campaign_watch:
            lines.append('- Campaign escalation watchlist:')
            for src, score in sorted(
                campaign_watch.items(),
                ...
                try:
                    ...
                    score_val = 0.0
                lines.append(f"  * {src} (score {score_val:.2f})")
            campaign_flag = True
        else:
            lines.append('- Campaign escalation watchlist: none flagged')

        surge_sources = data.get('surge_sources') or {}
        surge_flag = False
        if isinstance(surge_sources, dict) and surge_sources:
            lines.append('- Traffic surge sources:')
            for src, ratio in sorted(
                surge_sources.items(),
                key=lambda item: safe_float(item[1]),
                reverse=True,
            )[:3]:
                ratio_val = safe_float(ratio)
                lines.append(f"  * {src} (x{ratio_val:.2f} acceleration)")
            surge_flag = True
        else:
            lines.append('- Traffic surge sources: none observed')

        spike_sources = data.get('probability_spike_sources') or {}
        spike_flag = False
        if isinstance(spike_sources, dict) and spike_sources:
            lines.append('- Probability spike leaders:')
            for src, spike in sorted(
                spike_sources.items(),
                key=lambda item: safe_float(item[1]),
                reverse=True,
            )[:3]:
                spike_val = safe_float(spike)
                lines.append(f"  * {src} (+{spike_val:.2f} above baseline)")
            spike_flag = True
        else:
            lines.append('- Probability spike leaders: none observed')

        intensity_watch = data.get('intensity_watchlist') or {}
        intensity_flag = False
        if isinstance(intensity_watch, dict) and intensity_watch:
            lines.append('- Intensity watchlist:')
            for src, score in sorted(
                intensity_watch.items(),
                key=lambda item: safe_float(item[1]),
                reverse=True,
            )[:3]:
                score_val = safe_float(score)
                lines.append(f"  * {src} (intensity {score_val:.2f})")
            intensity_flag = True
        else:
            lines.append('- Intensity watchlist: none observed')

        role_counts = data.get('role_counts') or {}
        if isinstance(role_counts, dict) and role_counts:
            lines.append('- Behavioural roles detected:')
            for role, count in sorted(
                role_counts.items(),
                key=lambda item: safe_int(item[1]),
                reverse=True,
            ):
                lines.append(f"  * {role}: {safe_int(count)} sources")
        else:
            lines.append('- Behavioural roles detected: none yet')
        ...
        if fanout_flag:
            recommendations.append(
                'Prioritise containment for high fan-out scanners before they map the network.'
            )
        if port_flag:
            recommendations.append(
                'Review authentication logs for brute-force attempts across diverse ports.'
            )
        if surge_flag:
            recommendations.append(
                'Throttle or isolate hosts exhibiting sudden traffic acceleration.'
            )
        if spike_flag:
            recommendations.append(
                'Inspect payloads from sources showing sharp probability spikes.'
            )
        if intensity_flag:
            recommendations.append(
                'Escalate incident response for actors on the intensity watchlist.'
            )
        if campaign_flag:
            recommendations.append(
                'Open a campaign investigation ticket for hosts with elevated campaign scores.'
            )
        if recommendations:
            lines.append('')
...
run_engagement_simulation_menu() {
    if ! command -v python3 >/dev/null 2>&1 || [ ! -x /usr/local/bin/engagement_simulation.py ]; then
        show_msg "Engagement simulator is not available"
        return
    fi
    local scenario_args=()
    local scenario_catalog="/etc/nn_ids/engagement_scenarios.json"
    if [ -f "$scenario_catalog" ]; then
        scenario_args+=("--scenario-file" "$scenario_catalog")
    fi
    local listing
    listing=$(python3 /usr/local/bin/engagement_simulation.py "${scenario_args[@]}" --list --format tsv 2>/dev/null)
    if [ -z "$listing" ]; then
        show_msg "No engagement scenarios found"
        return
    fi
    local scenario
    if [ "$USE_DIALOG" -eq 1 ]; then
        ...
        scenario=$(dialog --backtitle "Kali IDS" --title "Engagement Simulation" --menu "Select a scenario:" 20 75 10 "${options[@]}") || return
    else
        ...
        while IFS=$'\t' read -r name desc; do
            printf "%d) %s - %s\n" "$idx" "$name" "$desc"
            scenario_names[$idx]="$name"
            idx=$((idx + 1))
        done <<<"$listing"
        read -rp "Choose a scenario: " idx
        scenario=${scenario_names[$idx]:-$idx}
        if [ -z "$scenario" ]; then
            show_msg "Invalid selection"
            return
        fi
    fi
    mkdir -p /opt/nnids/engagements
    local tmp
    tmp=$(mktemp)
    if python3 /usr/local/bin/engagement_simulation.py "${scenario_args[@]}" --scenario "$scenario" --output-dir /opt/nnids/engagements >"$tmp" 2>&1; then
        view_file "$tmp" "Engagement Simulation"
    else
        view_file "$tmp" "Simulation Error"
    fi
    rm -f "$tmp"
}

run_engagement_analysis() {
    if ! command -v python3 >/dev/null 2>&1 || [ ! -x /usr/local/bin/engagement_post_analysis.py ]; then
        show_msg "Engagement analysis tooling is not available"
        return
    fi
    mkdir -p /opt/nnids/analysis
    local tmp
    tmp=$(mktemp)
    if python3 /usr/local/bin/engagement_post_analysis.py --latest-simulation --analysis-dir /opt/nnids/analysis --stdout >"$tmp" 2>&1; then
        view_file "$tmp" "Engagement Analysis"
    else
        view_file "$tmp" "Analysis Error"
    fi
    rm -f "$tmp"
}

view_latest_engagement_report() {
    local dir="/opt/nnids/analysis"
    local latest
    latest=$(ls -1t "$dir"/engagement_report_*.md 2>/dev/null | head -n1)
    if [ -z "$latest" ]; then
        show_msg "No engagement analysis reports found"
    else
        view_file "$latest" "Engagement Report"
    fi
}
...
alert_analytics_menu() {
    while true; do
        local choice
        if [ "$USE_DIALOG" -eq 1 ]; then
            choice=$(dialog --backtitle "Kali IDS" --title "Alert Analytics" --menu "Select an action:" 20 78 9 \
                1 "View aggregated statistics" \
                2 "View recent alert history" \
                3 "View next-gen threat insights" \
                4 "Run engagement simulation" \
                5 "Run engagement analysis" \
                6 "View latest engagement report" \
                7 "Export analytics snapshot" \
                8 "Reset alert statistics" \
                9 "Return" 3>&1 1>&2 2>&3) || break
        else
            ...
            echo "4) Run engagement simulation"
            echo "5) Run engagement analysis"
            echo "6) View latest engagement report"
            echo "7) Export analytics snapshot"
            echo "8) Reset alert statistics"
            echo "9) Return"
...
generate_incident_report() {
    if [ ! -f "$INCIDENT_SCRIPT" ]; then
        show_msg "Incident response module not installed"
        return
    fi
    local tmp err_output report_path
    tmp=$(mktemp)
    if report_path=$(python3 "$INCIDENT_SCRIPT" --output "$INCIDENT_REPORT" 2>"$tmp"); then
        report_path=$(echo "$report_path" | tail -n1 | tr -d '\r')
        if [ -f "$report_path" ]; then
            view_file "$report_path" "Incident Response Report"
        else
            show_msg "Report generated but missing at $report_path"
        fi
    else
        err_output=$(cat "$tmp")
        if [ -n "$err_output" ]; then
            show_msg "Incident report failed: $err_output"
        else
            show_msg "Incident report generation failed"
        fi
    fi
    rm -f "$tmp"
}
...
launch_dashboard() {
    local script="/usr/local/bin/ids_dashboard_gui.py"
    if [ ! -x "$script" ]; then
        script="$(dirname "$0")/ids_dashboard_gui.py"
    fi
    if [ -x "$script" ]; then
        ...
        show_msg "Graphical dashboard closed"
    else
        show_msg "ids_dashboard_gui.py not found"
    fi
}
...
while true; do
    ...
    if [ "$USE_DIALOG" -eq 1 ]; then
        choice=$(dialog --clear --colors --backtitle "Kali IDS" --title "IDS Dashboard" --menu "Select an option:" 20 75 37 \
            ...
            48 "Generate incident response report" \
            49 "Launch graphical dashboard" \
            50 "Exit" 3>&1 1>&2 2>&3) || break
    else
        clear
        ...
        echo -e "${GREEN}48)${RESET} Generate incident response report"
        echo -e "${GREEN}49)${RESET} Launch graphical dashboard"
        echo -e "${GREEN}50)${RESET} Exit"
        read -rp "Choose an option: " choice
    fi
    case "$choice" in
        1)
