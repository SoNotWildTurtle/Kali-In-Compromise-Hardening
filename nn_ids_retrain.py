...
def main():
    ...
    adv_path = DATA_DIR / 'datasets' / 'adversarial.csv'
    if adv_path.exists():
        try:
            adv_df = pd.read_csv(adv_path)
            training_cols = ["len", "ttl", "dport", "flags", "label"]
            if set(training_cols).issubset(adv_df.columns):
                df_base = pd.concat([df_base, adv_df[training_cols]], ignore_index=True)
            else:
                missing = set(training_cols) - set(adv_df.columns)
                print(f"Warning: adversarial dataset missing columns: {sorted(missing)}")
        except Exception as exc:
            print(f"Warning: unable to load adversarial dataset: {exc}")
    ...
    with open('/var/log/nn_ids_train.log', 'a') as log:
        log.write(
            f"Retrain accuracy: {acc:.2f} f1: {f1:.2f} precision: {prec:.2f} "
