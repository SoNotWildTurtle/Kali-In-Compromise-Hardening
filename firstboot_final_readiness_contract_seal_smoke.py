#!/usr/bin/env python3
# MINC - Defensive final-readiness contract seal smoke gate.
# Purpose: validate aggregate contract-seal .summary.env evidence without sourcing shell.
...
DEFAULT_INPUT = Path('/var/log/firstboot_release_gate.final_readiness_contract_seal.summary.env')
SOURCE_PREFIX = 'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_'
SUMMARY_PREFIX = 'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SMOKE'
EXPECTED_SOURCE_COMPONENT = 'firstboot_final_readiness_contract_seal'
EXPECTED_SOURCE_PRIVACY_SCOPE = 'aggregate_firstboot_final_readiness_contract_seal_only'
EXPECTED_PRIVACY_SCOPE = 'aggregate_firstboot_final_readiness_contract_seal_only'
REQUIRED_KEYS = {
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_OK',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_DECISION',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_COMPONENT',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_DECISION',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_RELEASE_GATE',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_BLOCKER_COUNT',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_BLOCKERS',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_EXPECTED_ARTIFACTS',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_PRIVACY_SCOPE',
    'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SAFE_DEFAULT',
}
...
def read_env(path: Path) -> tuple[dict[str, str], list[str]]:
    ...
    try:
        ...
        return {}, [f'missing_contract_seal_summary:{path}']
    except UnicodeDecodeError as exc:
        return {}, [f'invalid_contract_seal_summary_encoding:{exc.reason}']
...
def build_report(values: dict[str, str], initial_issues: list[str]) -> dict[str, Any]:
    ...
    ok_value = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_OK', 'missing')
    decision = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_DECISION', 'missing')
    release_gate = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_RELEASE_GATE', 'missing')
    source_component = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_COMPONENT', 'missing')
    source_decision = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_DECISION', 'missing')
    source_release_gate = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_RELEASE_GATE', 'missing')
    source_privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SOURCE_PRIVACY_SCOPE', 'missing')
    privacy_scope = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_PRIVACY_SCOPE', 'missing')
    safe_default = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_SAFE_DEFAULT', 'missing')
    blockers = values.get('FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_BLOCKERS', 'missing')
    blocker_count = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_BLOCKER_COUNT', issues)
    expected_artifacts = as_nonnegative_int(values, 'FIRSTBOOT_FINAL_READINESS_CONTRACT_SEAL_EXPECTED_ARTIFACTS', issues)
    ...
    return {
        'schema_version': 1,
        'component': 'firstboot_final_readiness_contract_seal_smoke',
        'ok': approved,
        ...
        'operator_summary': 'Final readiness contract seal smoke evidence is approved.' if approved else f'Final readiness contract seal smoke evidence is deferred with {len(unique_issues)} blocker(s).',
        'privacy_scope': 'aggregate_firstboot_final_readiness_contract_seal_smoke_only',
        'safe_default': 'read-only final readiness contract seal smoke helper; no live system state was changed',
        'rollback_note': 'remove this optional contract seal smoke helper or stop packaging it; final-readiness contract seal JSON, Markdown, and summary evidence remain authoritative',
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        '# Firstboot final readiness contract seal smoke',
        '',
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate passive aggregate firstboot final-readiness contract seal summary evidence.')
